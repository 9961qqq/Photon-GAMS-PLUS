...
float get_water_caustics() {
	...
	vec3 normal = tbn * get_water_normal(world_pos, tbn[2], coord, flow_dir, 1.0 - rainStrength, flowing_water);
...
void main() {
	...
	if (material_mask == 1) { // Water
		...
		shadowcolor0_out = clamp(0.25 * exp(-absorption_coeff * distance_through_water) * get_water_caustics(), rcp(255.0) /* 0 is reserved */, 1.0);
